...
public class UnweightedGraph<V> implements Graph<V> {
	...
	/**Algorithm for assignment 28-3. This method uses depth first search without using recursion */
	    ...
	    while (!stack.isEmpty()) {
	        ...
	        if (neighbors.get(x).size() == 0) {
	        	...
	        	for (int i = 0; i <= neighbors.get(x).size() -1; i++) {
	        		...
	        		if (!isVisited[e.v]) {
	        			parent[e.v] = x;
	        			stack.push(e.v); // Push neighbor onto stack
	             
	        			isVisited[e.v] = true; // Mark neighbor as visited
	        			searchOrder.add(e.v);
	        			break;
	        		}
	        	}

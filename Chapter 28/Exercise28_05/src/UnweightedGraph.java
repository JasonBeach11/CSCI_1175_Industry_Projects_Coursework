...
public class UnweightedGraph<V> implements Graph<V> {
	...
	public List<Integer> getPath(int u, int v) {
		SearchTree tree = bfs(u);			// Perform a BFS starting from the vertex u
		List<V> path = tree.getPath(v);		// Get the path from vertex u to v
		
		// Convert the path vertices to a list of indices
		List<Integer> indices = new ArrayList<>();
		for (V vertex : path) {
			indices.add(getIndex(vertex));
		}
		// Reverse the order of the indices so they are returned in the correct order
